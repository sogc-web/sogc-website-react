function MobileMenuButton({ isOpen, onClick, label = 'Toggle menu' }) {
  return (
    ...
    >
      <span className="pointer-events-none absolute inset-0 -z-10 rounded-full bg-white/10 opacity-0 blur-md transition-opacity duration-300 ease group-hover:opacity-100" />
      <span
        className={`absolute h-0.5 w-[22px] rounded-full bg-white transition-all duration-300 ease ${
